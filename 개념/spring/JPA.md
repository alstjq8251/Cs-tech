...
3. `난이도`
  - jpa는 내부적으로 JDBC Api를 사용하여 db와 커넥션을 맺고 데이터를 가져오게 되는데 이러한 내부 동작 , JPA가 성능 향상을 꾀하는 원리등을 이해하는것은 쉽지 않다.
    - 논리적인 형태로 존재하는 영속성 컨텍스트 , 영속성 ,  DB를 지원하는 쓰기지연 저장소 역할도 이해해야 하며 때로는 jpa가 JDBC Api를 통해 쿼리를 만드는 과정과

      그를 통해 어떻게 통신하게 되는지를 이해하고 쓰지 못한다면 쿼리를 작성해주는 편리함에 취해 성능 , 기댓값과 다른 결과를 만들어 낼 수 있다.
#### JPA 내부 동작 방식(영속성)
